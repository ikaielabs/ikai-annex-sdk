/**
 * @file annex_hal_spi_st.c
 * @brief STM32 implementation for annex_hal_spi.h using STM HAL APIs.
 *
 ...
 * Wraps STM32 HAL SPI_HandleTypeDef with Annex-specific metadata.
 */
struct annex_hal_spi_dev {
    SPI_HandleTypeDef        hal_handle;     /* STM32 HAL SPI handle */
    annex_hal_spi_config_t   cfg;            /* Configuration copy */
...
#define ANNEX_HAL_SPI_MAX_DEVS 4U
...
int annex_hal_spi_enable(annex_hal_spi_dev_t *dev)
{
    ...
    hspi->Init.Mode = (dev->cfg.role == ANNEX_HAL_SPI_MASTER) ?
    ...
    hspi->Init.FirstBit = (dev->cfg.bit_order == ANNEX_HAL_SPI_LSB_FIRST) ?
...
void annex_hal_spi_enable_irq(annex_hal_spi_dev_t *dev, uint32_t irqs)
{
    ...
    if (irqs & ANNEX_HAL_SPI_IRQ_RX_READY) {
        __HAL_SPI_ENABLE_IT(hspi, SPI_IT_RXNE);
    }

    if (irqs & ANNEX_HAL_SPI_IRQ_TX_EMPTY) {
        __HAL_SPI_ENABLE_IT(hspi, SPI_IT_TXE);
    }

    if (irqs & (ANNEX_HAL_SPI_IRQ_ERROR | ANNEX_HAL_SPI_IRQ_TRANSFER_DONE)) {
...
void annex_hal_spi_disable_irq(annex_hal_spi_dev_t *dev, uint32_t irqs)
{
    ...
    if ((irqs & ANNEX_HAL_SPI_IRQ_RX_READY) && !(dev->irq_mask & ANNEX_HAL_SPI_IRQ_RX_READY)) {
        __HAL_SPI_DISABLE_IT(hspi, SPI_IT_RXNE);
    }

    if ((irqs & ANNEX_HAL_SPI_IRQ_TX_EMPTY) && !(dev->irq_mask & ANNEX_HAL_SPI_IRQ_TX_EMPTY)) {
        __HAL_SPI_DISABLE_IT(hspi, SPI_IT_TXE);
    }

    if ((irqs & (ANNEX_HAL_SPI_IRQ_ERROR | ANNEX_HAL_SPI_IRQ_TRANSFER_DONE)) &&
        !(dev->irq_mask & (ANNEX_HAL_SPI_IRQ_ERROR | ANNEX_HAL_SPI_IRQ_TRANSFER_DONE))) {
...
static void spi_rx_callback(SPI_HandleTypeDef *hspi)
{
    ...
    for (size_t i = 0; i < ANNEX_HAL_SPI_MAX_DEVS; i++) {
        if (s_dev_pool[i].in_use && &s_dev_pool[i].hal_handle == hspi) {
            s_dev_pool[i].irq_status |= ANNEX_HAL_SPI_IRQ_RX_READY;
            break;
        }
    }
}
...
static void spi_tx_callback(SPI_HandleTypeDef *hspi)
{
    ...
    for (size_t i = 0; i < ANNEX_HAL_SPI_MAX_DEVS; i++) {
        if (s_dev_pool[i].in_use && &s_dev_pool[i].hal_handle == hspi) {
            s_dev_pool[i].irq_status |= ANNEX_HAL_SPI_IRQ_TX_EMPTY;
            break;
        }
    }
}
...
static void spi_error_callback(SPI_HandleTypeDef *hspi)
{
    ...
    for (size_t i = 0; i < ANNEX_HAL_SPI_MAX_DEVS; i++) {
        if (s_dev_pool[i].in_use && &s_dev_pool[i].hal_handle == hspi) {
            s_dev_pool[i].irq_status |= ANNEX_HAL_SPI_IRQ_ERROR;
