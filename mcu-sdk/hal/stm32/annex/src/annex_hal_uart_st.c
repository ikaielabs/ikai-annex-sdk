...
#define ANNEX_HAL_UART_MAX_DEVS 4U

static struct annex_hal_uart_dev s_dev_pool[ANNEX_HAL_UART_MAX_DEVS];

static uint32_t annex_hal_uart_map_parity(uint8_t parity)
{
    switch (parity) {
    case 1:
...
static uint32_t annex_hal_uart_map_stop_bits(uint8_t stop_bits)
{
    return (stop_bits == 2U) ? UART_STOPBITS_2 : UART_STOPBITS_1;
}

static uint32_t annex_hal_uart_map_word_length(uint8_t data_bits)
{
...
annex_hal_uart_dev_t *annex_hal_uart_open(const annex_hal_uart_hw_desc_t *hw)
{
    ...
    for (size_t i = 0; i < ANNEX_HAL_UART_MAX_DEVS; i++) {
...
int annex_hal_uart_config(annex_hal_uart_dev_t *dev, const annex_hal_uart_config_t *cfg)
{
    ...
    dev->hal_handle.Init.WordLength = annex_hal_uart_map_word_length(cfg->data_bits);
    dev->hal_handle.Init.StopBits = annex_hal_uart_map_stop_bits(cfg->stop_bits);
    dev->hal_handle.Init.Parity = annex_hal_uart_map_parity(cfg->parity);
...
/*
 * annex_hal_uart_lin_init() is the proper entry point into LIN mode on
 ...
 *     annex_hal_uart_config() (baud rate, word length, etc.)
 ...
 * annex_hal_uart_lin_init() for actually entering LIN mode; passing
...
int annex_hal_uart_set_lin_mode(annex_hal_uart_dev_t *dev, bool enable)
{
    ...
    if (enable) {
        return annex_hal_uart_lin_init(dev, 11U);
    }
 ...
 * configured -- annex_hal_uart_lin_init() must have been called once,
...
void annex_hal_uart_disable_irq(annex_hal_uart_dev_t *dev, uint32_t irqs)
{
    ...
    if ((irqs & ANNEX_HAL_UART_IRQ_RX_READY) && !(dev->irq_mask & ANNEX_HAL_UART_IRQ_RX_READY)) {
        __HAL_UART_DISABLE_IT(&dev->hal_handle, UART_IT_RXNE);
    }
    if ((irqs & ANNEX_HAL_UART_IRQ_TX_EMPTY) && !(dev->irq_mask & ANNEX_HAL_UART_IRQ_TX_EMPTY)) {
...
uint32_t annex_hal_uart_get_irq_status(annex_hal_uart_dev_t *dev)
{
    ...
    if ((READ_BIT(usart->SR, USART_SR_RXNE) != 0U)) {
        status |= ANNEX_HAL_UART_IRQ_RX_READY;
    }
    if ((READ_BIT(usart->SR, USART_SR_TXE) != 0U)) {
        status |= ANNEX_HAL_UART_IRQ_TX_EMPTY;
    }
    if ((READ_BIT(usart->SR, USART_SR_ORE) != 0U)) {
        status |= ANNEX_HAL_UART_IRQ_RX_OVERRUN;
    }
