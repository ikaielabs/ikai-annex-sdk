...
int main(void)
{
  ...
  uart2 = annex_hal_uart_open(&uart_hw);

  annex_lin_hw_desc_t lin_hw = {
      .uart_handle = uart2
  };
  ...
  lin_handle = annex_lin_open(&lin_hw);
  ...
  if(annex_lin_read_message_wait(lin_handle, &id, rx_data, RX_LENGTH, &flags, 0xFFFFFF) == LIN_OK) {
	printf("Rx 0x%x {", id);
